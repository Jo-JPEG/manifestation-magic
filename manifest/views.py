...
@login_required
def home(request):
    manifestations = Manifestation.objects.filter(owner=request.user).order_by(
        "created_on"
    )

    for manifestation in manifestations:
        if (
            manifestation.is_charged
            and timezone.now()
            > manifestation.last_charged + timedelta(hours=24)
        ):
            manifestation.is_charged = False
            manifestation.save()
        if (
            manifestation.last_charged
            and timezone.now()
            > manifestation.last_charged + timedelta(hours=12)
        ):
...
def view_manifestation(request, slug):
    ...
    if manifestation.owner != request.user and not (
        manifestation.is_public and manifestation.is_approved
    ):
        return render(request, "manifest/forbidden.html")

    if (
        manifestation.is_charged
        and timezone.now() > manifestation.last_charged + timedelta(hours=24)
    ):
        manifestation.is_charged = False
        manifestation.save()
    if (
        manifestation.last_charged
        and timezone.now() > manifestation.last_charged + timedelta(hours=12)
    ):
        manifestation.can_charge = True
        manifestation.save()
    context = {
    ...
    return render(
        request,
        "manifest/view_manifestation.html",
        {"manifestation": manifestation},
    )
...
def edit_manifestation(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.add_message(
                request,
                messages.SUCCESS,
                "Your manifestation has been updated successfully. If public, it will be reviewed by an admin.",
            )
            return redirect("view_manifestation", slug=manifestation.slug)
    else:
    ...
    return render(
        request,
        "manifest/edit_manifestation.html",
        {"form": form, "manifestation": manifestation},
    )
...
def delete_manifestation(request, slug):
    ...
    if request.method == "POST":
        ...
        messages.add_message(
            request,
            messages.SUCCESS,
            "Your manifestation has been deleted successfully.",
        )
        return redirect("home")  # Redirect to a different view after deletion
    return render(
        request,
        "manifest/delete_manifestation.html",
        {"manifestation": manifestation},
    )
...
def public_manifestations(request):
    ...
    return render(
        request,
        "manifest/public_manifestations.html",
        {"manifestations": manifestations},
    )
